...
public class YarismaActivity extends AppCompatActivity {
    ...
    public void YarisSecim(View v){
        if(v.getId()==R.id.button_yaris){
            ...
            appMain.uploadToFirebase(SendDatas());*/
            uploadToFirebase(SendDatas());
            Intent intent = new Intent(this,YarisBaslat.class);
    ...
    private void uploadToFirebase(ArrayList<String> data){
        ...
        for(int i=0; i<data.size();i+=2){
            ...
            myRef.child("Dictionary").child("Race").child(kullanici.getString("username",null).toString()).child(uuid).child("Word").setValue(data.get(i));
            myRef.child("Dictionary").child("Race").child(kullanici.getString("username",null).toString()).child(uuid).child("Meaning").setValue(data.get(i+1));
        }
    }
        ...
        downRef.addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                for(DataSnapshot ds: dataSnapshot.getChildren()){
                    ...
                    /*downloadList.add(hashMap.get("Word"));
                    downloadList.add(hashMap.get("Meaning"));*/
